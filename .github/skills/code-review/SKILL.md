---
name: code-review
description: "Review pull requests and diffs in octo-analyser — the dockerised web security reconnaissance toolkit — against the rios0rios0/guide standards, with extra weight on Dockerfile hardening, version pinning, and keeping scan targets parameterised rather than hard-coded. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `octo-analyser`

`octo-analyser` orchestrates Nmap, Nikto, Wfuzz and (as placeholders) Arachni, Dirsearch and Reconnoitre as Docker services driven by a `TARGET_HOST` variable. It is offensive tooling for authorised testing, so the review watches both container hygiene and the risk of a target being baked in.
...
## What matters most in `octo-analyser`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

- **No target may be hard-coded.** Every scan is driven by `TARGET_HOST`; a hostname, IP, or URL committed as a default points the toolkit at someone's infrastructure by accident. **Critical.**
- **One tool per directory, one Dockerfile each**, with `docker-compose.yml` referencing each subdirectory as its build context. A tool added outside that shape breaks the pattern.
- **Versions are pinned deliberately** — Nmap 7.80 is compiled from the official tarball precisely so the version is exact. A floating base image or an unpinned package install makes results non-reproducible; a downloaded tarball needs its checksum verified.
- **Containers run as non-root wherever the tool allows it** — Nikto already does. A new image that runs as root needs a justification, and Hadolint findings are fixed rather than ignored.
- **Build dependencies are stripped from the final image.** These images carry attack tooling; leaving a compiler and headers in them widens what an escape would gain.
- **`arachni/`, `dirsearch/`, and `reconnoitre/` are placeholders.** Do not report them as broken; do reject a compose entry that assumes they work.
- **Scan output must not be committed.** Results contain third-party infrastructure detail and belong outside the repository.
- **Documentation must keep the authorisation framing.** README examples target private ranges and lab hosts; an example pointed at a real public host is a finding.
...
There is no test suite. Verification is building each changed image and running it against a lab host you are authorised to scan — never a third-party target.
...
- Long first builds (5–15 minutes for Nmap, which compiles from source).
- The placeholder tool directories.
